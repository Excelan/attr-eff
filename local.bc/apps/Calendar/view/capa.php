...
echo date('Y-m-d H:i',$thiscapa->eventtime);
        ...
        <div class="contentblock">
            <div class="leftblock">
                ...
                <div class="eventDetails">
                    ...
                    <section class="inputwidget">
                        <main class="BLK">
                            <label>Место проведения</label>
                            <div class="stack" data-containfieldtype="string" data-containfieldselector="title">
                                <div class="inputWithControls BLK item1" data-fieldtype="string" data-editmode="allow">
                                    <nav class="R"></nav>
                                    <div class="fieldbox">
                                        <input type="text" data-selector="title" id="eventPlace" <?if($subject->eventplace) echo "value='".$subject->eventplace."'"?>>
                                    </div>
                                </div>
                            </div>
                        </main>
                    ...
                    <section class="inputwidget">
                        <main class="BLK">
                            ...
                            <div class="stack" data-containfieldtype="date" data-containfieldselector="effectivedate">
                                <div class="inputWithControls BLK item1" data-fieldtype="date" data-editmode="allow">
                                    <nav class="R"></nav>
                                    <div class="fieldbox">
                                        <input type="time" id="eventTime" <?if($subject->eventtime) echo "value='".date('H:i',$subject->eventtime)."'"?>>
                                    </div>
                                </div>
                            </div>
                        </main>
...
<input id="checkDate" type="hidden" value="<?if(!is_null($thiscapa->$_GET['datefield'])) echo date('d',$thiscapa->$_GET['datefield']);?>">

<script>
    var urn = "<?=$surn?>";
    var gate = "/Events/GetUserCapaEnent";
    var month = "<?=$m?>";
    var year = "<?=$y?>";
    switchinginfo();
    ...
    var sendGate = '/Events/SetCapaDay';
    sendDataUserCapa(sendGate);

    goNextStage();

    //вывод сохраненного поля даты (функция запускается через GetNecessaryUser)
    //setSelectDateField();
